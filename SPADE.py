...
class SPADE(_ab._sequentialPatterns):
    ...
    **Credits:**
    ...
    def _creatingItemSets(self):
        ...
        if isinstance(self._iFile, str):
            try:
                ...
                quit()
        if isinstance(self._iFile, _ab._pd.Series):
            if self._iFile.empty:
                print("The series is empty")
            else:
                f = self._iFile.values
                for line in f:
                    line.strip()
                    temp = [i.rstrip() for i in line.split(self._sep)]
                    temp = [x for x in temp if x]
        ...
        for key1 in nextDatabase.keys():
            for key2 in nextDatabase[key1].keys():
                if len(nextDatabase[key1][key2].keys())>=self._minSup:
                    self._finalPatterns[str((key1,key2))]=len(nextDatabase[key1][key2].keys()) #changed
        ...
        for key1 in nextDatabaseSame.keys():
            for key2 in nextDatabaseSame[key1].keys():
                if len(nextDatabaseSame[key1][key2].keys()) >= self._minSup:
                    self._finalPatterns[str((key1,key2))]=len(nextDatabaseSame[key1][key2].keys()) # changed
    ...
    def makeNextRow(self,bs, latestWord, latestWord2):
        ...
        bs=bs+ (latestWord,) #changed
        bs2=bs+ (latestWord2,) # changed
    ...
    def makeNextRowSame(self,bs, latestWord, latestWord2):
        ...
        if len(bs)!=0:
            bs=tuple(bs)+tuple(x2) # changed
        else:
            bs=tuple(x2)
        bs2=tuple(bs)+tuple(latestWord2) #changed
        return bs2,bs
